		...
		return(
			<div className='bbl-container'>
				<div className='bbl-section'>
					...
					<input  className='bbl-search' 
							type='text' 
							placeholder={placeholder} 
							onChange={this.filterList.bind(this)} />
					<div className='bbl-body'>
						{this.state.options.map((o, idx) => {
							return (<div key={`op_${idx}`} onClick={this.clickAdd.bind(this, o)}>
											<Bubble txt={o.name} type='adder'/></div>)})}
					</div>
				</div>
				<div className='bbl-section right'>
					...
					<div className='bbl-body'>
						{this.state.chosen.map((c, idx) => {
							return (<div key={`ch_${idx}`} onClick={this.clickDelete.bind(this, c)}>
											<Bubble txt={c.name} type='deleter'/></div>)})}
					</div>
				</div>
			</div>
		);
