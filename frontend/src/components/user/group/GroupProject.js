...
export class GroupProject extends Component {
  ...
  submitApplication () {
    ...
    if (question_resps) resps = question_resps.map(q => q.response)
  ...
  renderApplicantCTA () {
    ...
    let project_action = (
      <div className={view_app_css} onClick={openFunc}>{view_app_text}</div>
    )
  ...
  renderSave () {
    ...
    var saveRemoveButton = (
      <div>
        {!this.state.added &&
          <div
            className='lab-srch-project-adder lab-srch-project-action-label'
...
export const GroupProjectContainer = props => {
  let editor = isFaculty()
    ? <Editor superClick={props.addFunction} add />
    : null
