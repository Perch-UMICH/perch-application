...
class LabSearchProject extends Component {
  ...
  submitApplication = () => {
    ...
    createApplicationResponse(application)
      .then(resp => {
        if (resp.data) submitStudentApplicationResponse(resp.data.id)
      })
  ...
  renderModals () {
    return (
      <EditModal
        id={`${this.state.position.id}-apply`}
        wide
        actionName='submit'
        title={`Apply To ${this.state.position.title}`}
        modalAction={this.submitApplication}
      >
        <Apply
  ...
  render () {
    var applyButton = (
      <div className='lab-srch-project-apply lab-srch-project-action-label'>
        <a onClick={() => this.openModal(`${this.state.position.id}-apply`)}>
          Apply
        </a>
      </div>
    )
    ...
    return (
      <div key={this.props.position.id} className='lab-srch-project'>
        ...
        <div className='lab-srch-project-title-container'>
          <a
            className='truncate lab-srch-project-title'
            href={`prof-page/${this.props.position.lab_id}`}
          >
            {this.state.position.title}
          </a>
          {this.props.urop &&
            <span className='lab-srch-project-tag'>UROP</span>}
        </div>

        <div className='lab-srch-project-description'>
          {this.state.position.description}
          <span
            className={this.state.overflowDescription ? 'ellipsis' : 'hide'}
          >
            ...
          </span>
        </div>

        <div className='lab-srch-project-openings'>
          <b>{this.state.position.spots} </b>
          {this.state.position.spots - 1 ? 'spots' : 'spot'}
        </div>
